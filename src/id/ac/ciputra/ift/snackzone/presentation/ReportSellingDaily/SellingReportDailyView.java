 ...
 * @author Kevin
 */
public class SellingReportDailyView {
    ...
    private JButton backButton, viewDetailButton, viewReportButton, printButton;
    ...
    public void initialize() {
        ...
        SellingReportDailyModel s = new SellingReportDailyModel();
        Vector<Date> distincDate = s.getDistinctDate();
        Vector date = new Vector();
        Vector month = new Vector();
        Vector year = new Vector();

        for (int i = 0; i < distincDate.size(); i++) {
            if (!year.contains(distincDate.get(i).getYear() + 1900)) {
                year.add(distincDate.get(i).getYear() + 1900);
            }
        }
    ...
    public Vector getProduct() {
        SellingReportDailyModel sModel = new SellingReportDailyModel();
        return sModel.readAllProduct();
    }

    public void runReportByParameter(String databaseName, String userName, String password,
            String reportFile, Date sellingDate) {
        HashMap parameterMap = new HashMap();
        parameterMap.put("sellingDate", sellingDate);
        try {
...
//        SellingReportDailyView c = new SellingReportDailyView();
//    }
    ...
    private class viewReportButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if (dateBox.getSelectedIndex() != - 1
                ...
                int month = convertMonth(monthBox.getSelectedItem().toString());
                ...
                for (int i = 0; i < selling.size(); i++) {
                    if (selling.get(i).getSellingDate().getDate() == date
                            && selling.get(i).getSellingDate().getMonth() == month
                ...
                JOptionPane.showMessageDialog(contentPane,
                        "Please Select Date, Month, and Year");
            }
        }
    }
    ...
    private class printButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String reportFile = "D:/UC/Semester 4/Visual Programming/Tugas/"
                    + "SnackZone/Report/SellingReportDaily.jrxml";
            Date sellingDate = new Date(Integer.parseInt(yearBox.getSelectedItem().
                    toString()) - 1900,
                    convertMonth(monthBox.getSelectedItem().toString()),
                    Integer.parseInt(dateBox.getSelectedItem().toString()));
